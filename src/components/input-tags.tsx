...
const InputTags = React.forwardRef<HTMLInputElement, InputTagsProps>(
  ({ className, value, onChange, ...props }, ref) => {
    ...
    return (
      ...
      >
        {value.map((item) => (
          <Badge className="bg-accent hover:bg-primary" key={item} variant="secondary">
            {item}
            <Button
              variant="ghost"
              size="icon"
        ...
        <input
          className="flex-1 dark:placeholder:text-neutral-400 placeholder:text-neutral-500 outline-none"
