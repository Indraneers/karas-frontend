...
export function FilterIcon({
  ...
  return (
    <div className={cn(["bg-muted mr-1 p-0.5 rounded-sm", className])}>
      {children}
    </div>
  );
}
...
function RadioOption({
  ...
  option: { id: string; label: string; icon: LucideIcon; value: string };
  isSelected?: boolean;
  groupName: string;
  onClick?: () => void;
}) {
...
export function SalesPopupFilter() {
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent
        className="justify-items-stretch gap-4 grid sm:grid-cols-[1fr_1fr_auto_2fr] sm:grid-rows-4 p-4 w-[550px]"
        align="start"
      >
              ...
              <RadioOption
                ...
                onClick={() => {
                  if (option.value === paymentType) {
                    setPaymentType(PaymentType.NONE);
                    navigate({
                      search: { ...searchFilter, paymentType: undefined },
                    });
                  }
                }}
              />
              ...
              <RadioOption
                ...
                onClick={() => {
                  if (option.value === status) {
                    setStatus(StatusEnum.NONE);
                    navigate({
                      search: { ...searchFilter, status: undefined },
                    });
                  }
                }}
              />
