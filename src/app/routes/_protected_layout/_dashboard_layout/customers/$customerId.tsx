...
export const Route = createFileRoute(
  ...
  validateSearch: (search: Record<string, unknown>) => ({
    page: (search.page as number | undefined) ?? 0,
  ...
  useEffect(() => {
    void navigate({
      search: (prev: Record<string, unknown>) =>
        ({ ...prev, customerId }) as never,
      replace: true,
    });
  }, [customerId]);

  const [customerQuery, vehicleQuery] = useQueries({
    queries: [
      {
      ...
      <div className="gap-4 grid grid-cols-4">
        <div>
          ...
          <Card className="mt-2 px-1">
            <CardHeader>
              <TypographyH2>Vehicles</TypographyH2>
            </CardHeader>
            <CardContent>
      ...
      <div className="xl:block flex gap-4 w-full xl:w-auto">
        <Card className="px-1">
          <CardHeader className="flex flex-row justify-between items-center gap-4">
            <TypographyH2>{customerQuery.data?.name || "-"}</TypographyH2>
            <EditButton
              className="w-8 h-8"
      ...
      <div className="col-span-3">
        <Card>
          <CardContent className="mt-4">
            <SalesTable queryKey={["sales-customer"]} />
          </CardContent>
        </Card>
      </div>
    </div>
